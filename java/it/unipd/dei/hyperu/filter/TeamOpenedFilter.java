 ...
 * @since 1.00
 */
public class TeamOpenedFilter extends AbstractFilter {
    ...
    public void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        // if not join request
        if (!(tokens != null && req.getMethod().equals("POST") && tokens.length == 2 && tokens[1].equals("request"))) {
            chain.doFilter(req, res);
            return;
        }

        try {
            Team team = TeamDAO.getTeamByIDMinimal(Integer.parseInt(tokens[0]));

            if (team != null) {
                // if the team is opened
                if (team.canAcceptRequests()) {
                    chain.doFilter(req, res);
                } else {
                    sendError(res, new ResultMessage(ErrorCode.TEAM_CLOSED));
                }
            } else {
