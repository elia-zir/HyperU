...
public class CommentFilter extends AbstractFilter {
    ...
    public void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        String[] tokens = getTokens(req, "comment");

        try {
            //Obtain the comment Object
            Comment comment = CommentDAO.getCommentByID(Integer.parseInt(tokens[0]));

            if (comment != null) {
                //if the logged user is the creator of the comment
                ...
                sendError(res, new ResultMessage(ErrorCode.NO_COMMENT_FOUND));
            }
