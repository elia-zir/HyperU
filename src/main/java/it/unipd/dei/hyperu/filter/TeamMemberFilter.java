 ...
 * @since 1.00
 */
public class TeamMemberFilter extends AbstractFilter {
    ...
    public void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if (team != null) {
                ...
                //if the logged user is a member of the group
                User loggedUser = getLoggedUser(req);
                boolean isTeamMember = TeamDAO.isTeamMember(team, loggedUser);

                if (isTeamMember) {
                    // If the user is not sending a join request
                    if (!(req.getMethod().equals("POST") && tokens.length == 2 && tokens[1].equals("request"))) {
                        chain.doFilter(req, res);
                    } else {
                        sendError(res, new ResultMessage(ErrorCode.USER_IS_MEMBER));
                    }
                    return;
                } else {
